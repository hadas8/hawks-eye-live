...
**Attempt policy: 3 submissions, per-ROUND feedback.** Briefly 4, when the station doubled; back to 3 on 2026-09-16 at Hadas's call, which also puts it level with stations 1, 6 and 7. Three is safe because the feedback is per round and never per truck: a round holds six binary answers, so learning that round two is wrong leaves 64 possibilities and there is nothing to sweep. Per-truck verdicts would let a group flip one card at a time and read the rule off the app instead of off the data.

**The round's ✓ / ✗ is the feedback, and no sentence repeats it.** The verdict used to name the failing round in prose — "סבב ב׳ לא נכון" — which was the only place in the app where a sentence did a mark's job; stations 1, 3, 6 and 7 all mark the part and say nothing further. It is now one generic line pointing at the marks. A round's mark clears as soon as any truck in it moves, because the verdict was about an answer that no longer exists.
