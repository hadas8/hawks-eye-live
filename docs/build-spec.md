...
5. ~~**Station 6 has no distance metric.**~~ **It does — `תחנה_6_מעברים.xlsx` states one the draft never had**, checked 2026-09-09: *מתחילים בשעת פעילות, ממשיכים לסוג כביש, כיסוי עצים, גובה, רכבים ביום — שני מעברים דומים = חולקים כמה שיותר תכונות מהתחלת הרשימה*. That is a lexicographic match, and it is checkable.
   ...
   **But running it does not give the digit. Built anyway, 2026-09-10.** Computed: N-01 = 2, N-02 = 3, N-03 = 2, N-04 = 3, summing to 10 and a digital root of **1**. The roster needs **2**, which requires the draft's 2 + 3 + 3 + 3 = 11. The disagreement is N-03: the draft asserts 3, the stated rule gives 2 and cannot give 3 under any choice of third neighbour, because its only close match is K-12 and every remaining daytime crossing scores 1 or 2. Rounding down gives 9, up gives 4, so no convention rescues the 2. **Hadas then dropped N-04 as a duplicate of N-02, which makes it 2 + 3 + 2 = 7 and the digit 7.** `CFG.lockCode` is now **`3294277`** — with station 3's 9 at position three, **two digits now differ from the draft's code and both move the physical lock.** The key in `src/data/station-6.js` is *computed* from the stated rule rather than typed, so it cannot drift from it.

   **Station 6 played far too hard, and three fixes were applied 2026-09-10 at Hadas's request:** N-04 dropped (48 comparisons down to 36), a rejected submission now names which crossings are wrong the way station 1 does, and the known table gained a filter **the group drives**: three buttons, one per matchable feature, each dimming the rows that fail the *active* crossing on it. Nothing dims until they switch one on.

   The first version applied שעת פעילות automatically and Hadas rejected it — *"the filter doesn't make any sense."* Two faults: it made the first decision before the group had acted, and no principle said step 1 should be automatic and steps 2 to 5 not. **This is the rule for anything like it: the app shows the consequence of a choice the group made, it does not make the choice.** Station 4's fleet preview is the same shape and is the precedent to follow.

   Only the three *matchable* features get a button. גובה and רכבים/יום get none, because nothing in the table sits at exactly 360 מ׳ — comparing those is the tie-break, done by eye. That also surfaces the source's blind spot instead of hiding it: its rule says crossings *share* features and then lists two that cannot be shared.
