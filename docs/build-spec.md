...
5. ~~**Station 6 has no distance metric.**~~ **It does — `תחנה_6_מעברים.xlsx` states one the draft never had**, checked 2026-09-09: *מתחילים בשעת פעילות, ממשיכים לסוג כביש, כיסוי עצים, גובה, רכבים ביום — שני מעברים דומים = חולקים כמה שיותר תכונות מהתחלת הרשימה*. That is a lexicographic match, and it is checkable.
   ...
   **Station 6 played far too hard, and three fixes were applied 2026-09-10 at Hadas's request:** N-04 dropped (48 comparisons down to 36), a rejected submission now names which crossings are wrong the way station 1 does, and the known table filters live to the crossing being worked on, dimming rows that fail its שעת פעילות — the first step of the rule and the mechanical half of the work, leaving every judgement alone. Filtering by road type as well would leave one or two rows standing and give away the top of the answer.
