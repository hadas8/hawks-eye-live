...
**3. The station is now capped at three attempts.** The doc assumes uncapped throughout and flags it only as a time risk ("submissions here are uncapped so a stuck group burns the whole slot"). Capped 2026-09-21 at Hadas's call, after the rewrite shipped. Three rather than two because **this is the only station in the app that gives no partial feedback at all** — station 4 is also `revealWhichWrong: false` but still shows how many each chosen question filtered, and every other station marks its parts — so an attempt buys less here, and a submission is six independent judgements graded together. Running out is not a dead end: the digit is still handed over so the box can open, and the station scores 0. The module had no cap handling at all before this, so it was wiring as well as a number.

**3a. Seven to six was free in code but not in the brute-force margin, and the roster comment said seven.** "This costs nothing" is right about `answer.parts`, `N` and the digit. It is not right about the one thing that depended on the count: station 2 takes **uncapped submissions**, justified in `src/data/stations.js` by "seven binary choices with no partial feedback gives nothing to hill-climb on, so a sweep costs more clicks than the clock allows." Seven is 128 blind guesses; six is 64 — several minutes of a seven-minute clock rather than more than all of it. The comment now carries the real arithmetic and a warning: **do not cut a fifth envelope without revisiting it.** Five would be 32. Superseded in part by the cap above, which closes the blind-resubmission route directly — but the warning stands, because a fifth cut would also change the odds the cap is sized against. This also answers the doc's own aside that ה׳ could replace ב׳ and the station could run at five.
