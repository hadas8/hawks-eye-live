...
Shipped at 3 attempts with the feedback on, because the unactionable rejection was the live problem and the brute-force is a hypothesis about how groups will behave. **If it needs closing, dropping to `maxAttempts: 2` closes it completely** — with two attempts the final move is a simultaneous guess between two boxes on every wrong card at once, which no group will land. Per-box error counts instead of per-card marks would also close it while staying actionable, at the cost of being much less satisfying to look at. Worth deciding after the station is play-tested rather than before.
