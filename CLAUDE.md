...
A one-off digital escape room for a live, in-person data science seminar run by the 8200 Alumni Association (Techlift). Six stations teach the intuition behind an ML concept before naming it; each yields one digit, and the six digits open the vault screen that ends the evening. **There is no physical box** — the seized crate is fiction and the vault screen is the whole payoff. It was seven stations until 2026-09-21; see the roster header in `src/data/stations.js`.
...
**All six stations are built, and all six passwords are final** — נוצה, אופק, מדף, ענף, מצפן, חורש. גדר retired with station 6 and is not reused. They were chosen here rather than supplied by the source, and Hadas adopted them as the real ones on 2026-09-16. Facilitators announce them from paper, so changing one means reprinting; do not touch them casually.

**Station 6 — ארבעה כוכבים, KNN — was removed on 2026-09-21 at Hadas's call**, taking the roster from seven stations to six and the lock code from `3274227` to `327427`: its digit, a 2, came out and nothing else moved. It was the only station that **handed the group its rule** instead of making them find it, which made it execution rather than insight, and none of the volume fixes touched that. Old station 7 became station 6. `docs/station-6-neighbours.md` keeps the analysis. **Never add or remove a station without moving `CFG.lockCode` with it** — `npm run check` is what catches that.

Stations 1, 4, 5 and 6 are verified and closed. **Station 2 was rewritten on 2026-09-21** — see `docs/station-2-rewrite.md`. Seven envelopes became six, and its chart data stopped being Lotem's on 2026-09-17 by Hadas's decision. The rule that keeps it working is in the header of `src/data/station-2.js`: one class of lie per envelope, the two charts of a pair differ in exactly one way, and **emphasis is not a lie** — before adding an envelope, name the false statement the lying chart makes. **Its six questions, hints and titles still need Hadas's Hebrew pass.** ב׳'s series was revised again the same day so no month is empty, which moved its false claim from continuity to level — do not restore the zeros without also restoring the old question. **Station 5 has a second round that is not from the source** — a two-feature rule written here on 2026-09-16 because the station ran in ninety seconds; its cards were verified by sweeping 144 candidate rules, and its concept is now `למידה מפוקחת` alone, which means unsupervised learning is previewed nowhere in the evening. Nothing is open with the content author any more: the last such question was station 6's undefined "closest", which left with the station.
