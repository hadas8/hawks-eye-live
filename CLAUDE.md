...
- **Station 3 is the second exception, and it is deliberate.** Its three boxes carry `--box-1/2/3`, and a card takes its box's colour when it is sent there. Colour marks a *place* rather than cutting across a scan, which is the opposite situation to the tables, and the source colour-codes the boxes for the same reason. Hadas's call, 2026-09-10. Do not generalise it to other stations without asking.
